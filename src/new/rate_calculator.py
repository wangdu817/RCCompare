...
def get_reaction_thermo_properties(reaction_data, T_kelvin, thermo_data_dict):
    ...
    for coeff, species_name in all_species_in_reaction:  # 修正：coeff在前，species_name在后
    ...
    # Products
    for coeff, species_name in reaction_data.get('products', []):  # 修正：coeff在前，species_name在后
    ...
    # Reactants
    for coeff, species_name in reaction_data.get('reactants', []):  # 修正：coeff在前，species_name在后
        H, S, G, Cp = get_thermo_properties(species_name, T_kelvin, thermo_data_dict)
...
def calculate_reverse_rate_constant(kf, Kc):
    """
    Calculate reverse rate constant from forward rate constant and equilibrium constant.
    kr = kf / Kc
    """
...
def merge_duplicate_reactions(parsed_reactions):
    """
    Merge duplicate reactions by combining their rate constants.
    
    For reactions marked with DUP keyword that have the same chemical equation,
    their rate constants should be added together: k_total = k1 + k2 + ...
    
    Args:
        parsed_reactions: List of reaction dictionaries from chemkin_parser
        
    Returns:
        List of merged reaction dictionaries with combined rate constants
    """
    if not parsed_reactions:
        return []
    
    # Group reactions by their normalized equation string
    reaction_groups = {}
    
    for reaction in parsed_reactions:
        # Create a normalized equation key for grouping
        eq_key = _normalize_equation_for_grouping(reaction)
        
        if eq_key not in reaction_groups:
            reaction_groups[eq_key] = []
        reaction_groups[eq_key].append(reaction)
    
    merged_reactions = []
    
    for eq_key, reactions_group in reaction_groups.items():
        if len(reactions_group) == 1:
            # Single reaction, no merging needed
            merged_reactions.append(reactions_group[0])
        else:
            # Multiple reactions with same equation - check if they should be merged
            duplicate_reactions = [r for r in reactions_group if r.get('is_duplicate', False)]
            non_duplicate_reactions = [r for r in reactions_group if not r.get('is_duplicate', False)]
            
            # Add non-duplicate reactions as-is
            merged_reactions.extend(non_duplicate_reactions)
            
            # Merge duplicate reactions
            if duplicate_reactions:
                merged_reaction = _merge_duplicate_reaction_group(duplicate_reactions)
                if merged_reaction:
                    merged_reactions.append(merged_reaction)
    
    return merged_reactions


def _normalize_equation_for_grouping(reaction):
    """
    Create a normalized equation string for grouping duplicate reactions.
    This ensures reactions with the same stoichiometry are grouped together.
    """
    # Use the cleaned equation string, but normalize case and spacing
    eq_str = reaction.get('equation_string_cleaned', reaction.get('equation_string', ''))
    
    # Normalize by converting to uppercase and removing extra spaces
    normalized = ' '.join(eq_str.upper().split())
    
    return normalized


def _merge_duplicate_reaction_group(duplicate_reactions):
    """
    Merge a group of duplicate reactions into a single reaction with combined rate constants.
    
    For Arrhenius reactions: k_total = k1 + k2 + ... = A1*T^n1*exp(-Ea1/RT) + A2*T^n2*exp(-Ea2/RT) + ...
    This is complex to represent as a single Arrhenius expression, so we store multiple parameter sets.
    
    For PLOG/TROE reactions: These are more complex and should be handled case by case.
    """
    if not duplicate_reactions:
        return None
    
    # Use the first reaction as the base
    base_reaction = duplicate_reactions[0].copy()
    
    # Collect all Arrhenius parameters for merging
    all_arrhenius_params = []
    all_raw_lines = []
    all_warnings = []
    
    for reaction in duplicate_reactions:
        if reaction.get('arrhenius_params') and reaction['arrhenius_params'].get('A') is not None:
            all_arrhenius_params.append(reaction['arrhenius_params'])
        
        # Collect raw lines and warnings
        all_raw_lines.extend(reaction.get('raw_lines', []))
        all_warnings.extend(reaction.get('warnings', []))
    
    # Update the base reaction
    base_reaction['raw_lines'] = all_raw_lines
    base_reaction['warnings'] = all_warnings
    base_reaction['is_duplicate'] = True
    
    # Store multiple Arrhenius parameter sets for rate calculation
    if len(all_arrhenius_params) > 1:
        base_reaction['duplicate_arrhenius_params'] = all_arrhenius_params
        # Keep the first set as the primary for compatibility
        base_reaction['arrhenius_params'] = all_arrhenius_params[0]
        
        # Add a note about the merging
        merge_note = f"Merged {len(duplicate_reactions)} duplicate reactions with DUP keyword"
        if merge_note not in base_reaction['warnings']:
            base_reaction['warnings'].append(merge_note)
    
    return base_reaction


def calculate_merged_duplicate_rate(reaction_data, T, P_atm=1.0):
    """
    Calculate the total rate constant for a merged duplicate reaction.
    
    For reactions with multiple Arrhenius parameter sets (from DUP merging),
    calculate k_total = k1 + k2 + ... where each ki is calculated separately.
    """
    if not reaction_data.get('duplicate_arrhenius_params'):
        # Not a merged duplicate reaction, use normal calculation
        return None
    
    total_rate = 0.0
    
    for arrhenius_params in reaction_data['duplicate_arrhenius_params']:
        try:
            k_individual = calculate_arrhenius_rate(arrhenius_params, T)
            if k_individual is not None and k_individual > 0:
                total_rate += k_individual
        except Exception:
