...
def _parse_coeff_line(line, num_coeffs):
    """
    智能解析包含科学计数法数字的行，处理连在一起的数字
    
    Args:
        line (str): 包含系数的行
        num_coeffs (int): 期望的系数数量
    
    Returns:
        list: 解析出的浮点数系数列表
    """
    # 去除行末的数字标识和空格
    line_clean = line[:75].strip()
    
    # 使用正则表达式匹配科学计数法数字
    # 匹配模式：可选的负号 + 数字 + 可选小数点和数字 + 可选科学计数法部分
    pattern = r'[+-]?\d+\.?\d*[eE][+-]?\d+|[+-]?\d+\.\d*|[+-]?\d+'
    matches = re.findall(pattern, line_clean)
    
    # 转换匹配的字符串为浮点数
    try:
        coeffs = [float(match) for match in matches]
...
def read_nasa_polynomials(filepath):
    ...
    for i, line_full in enumerate(lines):
        ...
        # Using a more flexible regex for elements (case-insensitive)
        match_header = re.match(
            r"^(?P<name>\S+)\s*"                                    # Species name
            r"(?P<date>\S+)?\s*"                                    # Optional date code
            r"(?P<elements>(?:[A-Za-z][a-z]?\s*\d+\s*)+)\s*"        # Elements and counts (case-insensitive)
            r"(?P<phase>[A-Za-z0-9])\s+"                            # Phase (G/g, L/l, S/s, or numbers)
            r"(?P<Tlow>\d+\.\d+)\s+"                                # Tlow
        ...
        if match_header and (len(line) < 79 or line[79] == '1'): # Line 1 of 4 for a species
            ...
            if len(current_species_data['coeff_lines_raw']) == 3: # We have all 3 coefficient lines
                ...
                line2_coeffs = _parse_coeff_line(current_species_data['coeff_lines_raw'][0], 5)
                coeffs_high_T.extend(line2_coeffs)

                # Line 3: a6_H, a7_H (High T), a1_L, a2_L, a3_L (Low T)
                line3_coeffs = _parse_coeff_line(current_species_data['coeff_lines_raw'][1], 5)
                coeffs_high_T.extend(line3_coeffs[:2])  # 前2个是高温系数
                coeffs_low_T.extend(line3_coeffs[2:5])  # 后3个是低温系数
                
                # Line 4: a4_L, a5_L, a6_L, a7_L (Low T)
                line4_coeffs = _parse_coeff_line(current_species_data['coeff_lines_raw'][2], 4)
                coeffs_low_T.extend(line4_coeffs)
...
def is_valid_nasa_polynomial_string(polynomial_string):
    ...
    for i in range(0, num_lines, 4):
        ...
        try:
            # Line 2: 5 coeffs
            line2_coeffs = _parse_coeff_line(line2, 5)
            coeffs_to_check.extend(line2_coeffs)
            
            # Line 3: 5 coeffs
            line3_coeffs = _parse_coeff_line(line3, 5)
            coeffs_to_check.extend(line3_coeffs)
            
            # Line 4: 4 coeffs
            line4_coeffs = _parse_coeff_line(line4, 4)
            coeffs_to_check.extend(line4_coeffs)
            ...
            error_messages.append(f"Entry {entry_num} ({species_name}): Coefficient parsing failed: {e}")
        except IndexError:
